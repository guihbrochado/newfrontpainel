...
const KanbanProvider = ({ children }: ChildrenType) => {
  ...
  const handleNewTask = newTaskHandleSubmit((values: TaskFormFields) => {
    if (activeSectionId) {
      const newTask: KanbanTaskType = {
        title: values.title || 'Untitled Task', // Garantir que title seja sempre uma string
        description: values.description || '', // Valor padrão para description
        priority: values.priority!, // O yup garante que priority não será undefined
        tags: [values.tags!], // O yup garante que tags não será undefined
        totalTasks: values.totalTasks || 0, // Valor padrão para totalTasks
        sectionId: activeSectionId,
        id: (Number(tasks.slice(-1)[0]?.id || 0) + 1).toString(), // Garantir que o ID seja gerado corretamente
        completedTasks: 0,
        commentsCount: 0,
      };
  ...
  const handleNewSection = sectionHandleSubmit((values: SectionFormFields) => {
    const section: KanbanSectionType = {
      id: (Number(sections.slice(-1)[0]?.id || 0) + 1).toString(),
